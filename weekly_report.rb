...
subscribers.each do |user|
  ...
  next unless user['locations']

  user['locations'].each do |location|
    next unless location['weekly']
    ...
    response = HTTParty.get("https://api.open-meteo.com/v1/forecast?latitude=#{latitude}&longitude=#{longitude}&daily=temperature_2m_max,temperature_2m_min,snowfall_sum,precipitation_probability_max,wind_speed_10m_max&timezone=America%2FLos_Angeles&forecast_days=7&wind_speed_unit=mph&temperature_unit=fahrenheit&precipitation_unit=inch")
    ...
    daily_data['time'].each_with_index do |date, index|
      ...
      email_body += "<hr style='margin:15px 0;'>"
    ...
    email_body += "<a href='#{all_unsub}'>Unsubscribe from ALL locations</a>"
    email_body += "</p>"

    Mail.deliver do
      from "deesjaime@gmail.com"
      to email
      subject "Weekly Ski Report — #{location_display}"
      content_type 'text/html; charset=UTF-8'
      body email_body
    end

    puts "Weekly email sent to #{email} (#{location_display})"
